...
if __name__ == "__main__":
    ...
    for fname in all_csv_files:
        ...
        try:
            ...
            p_metrics = ekf_evaluator.compute_parameter_metrics(Cs_arr, UA_arr, m_inf_arr)
            metrics.update(p_metrics)
            metrics["dataset"] = base_name
            metrics["day"] = day
            metrics["model"] = "EP Single EKF"
    ...
    if metrics_list:
        ...
        csv_out_p = os.path.join(SCRIPT_DIR, "ep_single_ekf_metrics.csv")
        df_metrics.to_csv(csv_out_p, index=False)
        print(f"\n=================================================================")
        print(f"  ENERGYPLUS SINGLE EKF SUMMARY METRICS (Saved to {csv_out_p})")
        print(f"=================================================================")
