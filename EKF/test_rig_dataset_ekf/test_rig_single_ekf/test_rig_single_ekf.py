"""
SmartBEM FYP — 10-State Single EKF for Experimental Test Rig (Day 3 & Day 4)
======================================================================
Fully aligned with advisor's EMS_Cookbook ODEs & ROBOD-consistent formulation:
  ...
  - Automated threshold optimization and evaluation metrics via ekf_evaluator.py

Generates exactly 4 PNG plots per dataset + outputs test_rig_single_ekf_metrics.csv.
"""
...
def get_ground_truth_occupancy(df_data, dataset_name):
    ...
    truth_csv_path = os.path.join(CLEAN_DATA_DIR, truth_filename)
...
if __name__ == "__main__":
    ...
    all_csv_files = sorted([f for f in os.listdir(CLEAN_DATA_DIR) if f.startswith("day_") and f.endswith(".csv")])

    print(f"\n=================================================================")
    print(f"  10-State Single EKF — Experimental Test Rig (Day 3 & Day 4)")
    ...
    for fname in all_csv_files:
        ...
        print(f"Running Single 10-State EKF for Day {day}: {base_name}...")
        ...
        ax.step(t_min, n_disc, color=COLOR_BLUE, where="post", linewidth=2.2, alpha=0.9, label="Thresholded Integer Occupants (≥0.35 Threshold)")
        ...
        for idx, (p_idx, label_str, color_str, title_str) in enumerate(param_configs):
            ax_p = axes[idx]
            ax_p.plot(t_min, X_hist[:, p_idx], color=color_str, linewidth=1.8, label=title_str)
            ax_p.set_ylabel(label_str, fontsize=9)
            ax_p.legend(loc="upper right", fontsize=8, frameon=True, facecolor="white")
        ...
        ax1.set_title(f"{base_name} — Single-EKF Derived Physical Building Parameters", fontsize=12, fontweight="bold", pad=12)
    ...
    if metrics_list:
        ...
        csv_out_p = os.path.join(SCRIPT_DIR, "test_rig_single_ekf_metrics.csv")
        df_metrics.to_csv(csv_out_p, index=False)
        print(f"\n=================================================================")
        print(f"  SINGLE EKF EVALUATION SUMMARY METRICS (Saved to {csv_out_p})")
        print(f"=================================================================")
