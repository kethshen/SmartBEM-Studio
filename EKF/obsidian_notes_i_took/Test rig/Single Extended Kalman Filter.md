...
### 1. Constant Parameter Dynamics ($\dot{\theta} = 0$)
All 7 building parameters are modeled as stochastic random-walk processes (driven by process noise matrix $Q$):
$$
\frac{d\alpha_o}{dt} = 0, \quad \frac{d\alpha_s}{dt} = 0, \quad \frac{d\alpha_e}{dt} = 0
$$
$$
\frac{d\beta_o}{dt} = 0, \quad \frac{d\beta_s}{dt} = 0, \quad \frac{d\beta_e}{dt} = 0
$$
$$
\frac{d\gamma_e}{dt} = 0
$$
...
$$
* **$\alpha_o (T_o - T_z)$**: Conductive heat exchange across polyurethane envelope walls ($\alpha_o = \frac{UA}{C_s}$).
* **$\alpha_s \cdot \dot{m}_{sa} \cdot (T_{sa} - T_z)$**: Sensible heat transfer rate from HVAC supply air ($\alpha_s = \frac{c_{pa}}{C_s}$).
* **$\alpha_e$**: Unmodeled internal heat generation bias ($[^\circ\text{C}/\text{s}]$).
...
$$
* **$\beta_o (\omega_o - \omega_z)$**: Outdoor moisture infiltration rate ($\beta_o = \frac{\dot{m}_{\text{inf}}}{M_{\text{room}}}$).
* **$\beta_s \cdot \dot{m}_{sa} \cdot (\omega_{sa} - \omega_z)$**: Supply air moisture mixing rate ($\beta_s = \frac{1}{M_{\text{room}}}$).
...
$$
* **$\beta_o (c_o - c_z)$**: Outdoor $\text{CO}_2$ ingress via envelope leaks.
* **$\beta_s \cdot \dot{m}_{sa} \cdot (c_{sa} - c_z)$**: Supply air $\text{CO}_2$ flushing/mixing rate.
* **$\gamma_e$**: Lumped human occupant $\text{CO}_2$ generation rate directly in $[\text{ppm}/\text{s}]$ ($\gamma_e = N_{\text{occ}} \cdot 0.7716\text{ ppm/s}$).
