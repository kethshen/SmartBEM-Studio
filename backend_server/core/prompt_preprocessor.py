...
class PromptPreprocessor:
    ...
    def restructure_prompt(self, raw_prompt, model_type="ollama"):
        ...
        system_prompt = (
            ...
            "   - roof: [type], gable_height=H.HH, gable_dir=[ew|ns], skylight=WxH (specify skylight width/height explicitly if present).\n"
            "     - type options: 'flat' | 'pitched' (gable) | 'pyramid' (hip all-slopes-to-apex)\n"
            "     - gable_dir: only for 'pitched'. 'ew' = ridge runs East-West (default). 'ns' = ridge runs North-South.\n"
            ...
            "- ROOF TYPE MAPPING: Use ONLY these exact strings for roof type: 'flat' (default), 'pitched' (gable roof with a ridge), 'pyramid' (hip/tent roof with 4 slopes meeting at a single point - use for square/near-square zones or when user says 'hip', 'pyramid', 'tent roof', 'cone roof'). For 'pitched', set gable_dir='ns' if the zone's length (X) >> width (Y) or if user says 'ridge runs N-S', 'NS ridge'; otherwise default gable_dir='ew'.\n\n"
            "=== TEMPLATE EXAMPLES ===\n"
