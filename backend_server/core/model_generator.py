...
"""
    ...
    def _extract_topology(self, nlp_text, config, model_type):
        system_prompt = (
            ...
            "     - 'roof_type' (string: 'flat', 'pitched', or 'pyramid'. Default 'flat'. Use 'pitched' for gable roofs, 'pyramid' for 4-slope hip roofs converging to a single apex point)\n"
            "     - 'gable_dir' (string: 'ew' or 'ns'. Only relevant when roof_type='pitched'. 'ew' = ridge runs East-West (default), 'ns' = ridge runs North-South. Use 'ns' for zones that are longer in the Y direction or when the user says the ridge faces north-south or perpendicular gable)\n"
