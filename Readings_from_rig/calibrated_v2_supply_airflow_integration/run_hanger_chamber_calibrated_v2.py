...
EPW_PATH = os.path.join(CALIBRATED_V2_DIR, "day_1_weather_merged_1min.epw")
CSV_CLEANED_PATH = os.path.join(CALIBRATED_V2_DIR, "day_1_p_1.csv")
ANEMOMETER_PATH = os.path.join(os.path.dirname(SCRIPT_DIR), "experimental_data", "fan_value_and_anemometer.csv")
SENSOR_FLOW_CSV_PATH = os.path.join(CALIBRATED_V2_DIR, "day_1_p_1_fan_flow_rate_raw.csv")
...
plt.plot(time_vector, Tz_ema, color="#2A9D8F", linewidth=2.5, label="Weighted Tz")
plt.plot(time_vector, best_Tsim, color="#FF6B6B", linestyle="--", linewidth=2.2, label="Calibrated Tz")

plt.title("calibrated_v2_supply_airflow_integration", fontsize=14, fontweight="bold", pad=15)
...
plt.title("calibrated_v2 — Outdoor Temperature Verification", fontsize=14, fontweight="bold", pad=15)
plt.xlabel("Elapsed Time (Minutes)", fontsize=12, labelpad=8)
...
ax1.set_title("calibrated_v2 — Supply Air Mass Flow Rate & Control Signals", fontsize=14, fontweight="bold", pad=15)
...
ax2.plot(np.linspace(0, 170, len(fan_pct_interp)), fan_pct_interp, color="#9D4EDD", linewidth=2.0, label="Fan Speed Control (%)")
ax2.plot(np.linspace(0, 170, len(mixer_pct_interp)), mixer_pct_interp, color="#264653", linestyle="-.", linewidth=2.0, label="Mixer Opening (%)")
...
# 6. Diagnostic Figure 1: calibrated_v2_evaluation_parameters.png (5 Error Subplots)
EVAL_PLOT_PATH = os.path.join(CALIBRATED_V2_DIR, "calibrated_v2_evaluation_parameters.png")
...
axes[0].set_title("calibrated_v2 — Optimization Convergence & Evaluation Metrics History", fontsize=13, fontweight="bold", pad=12)
...
# 7. Diagnostic Figure 2: calibrated_v2_parameter_trajectories.png (5 Parameter Subplots)
PARAM_PLOT_PATH = os.path.join(CALIBRATED_V2_DIR, "calibrated_v2_parameter_trajectories.png")
fig, axes = plt.subplots(5, 1, figsize=(12, 14), sharex=True, dpi=300)
...
axes[0].set_title("calibrated_v2 — Physical Parameter Calibration Trajectories", fontsize=13, fontweight="bold", pad=12)
axes[0].grid(True, linestyle=":", alpha=0.6)
