"""
Seaborn Raw vs. Cleaned Comparison Plot Generator — With Occupancy Datasets
===========================================================================
...
Subplots per channel:
  - Raw: Dashed line (ls="--", alpha=0.7, lw=1.2)
  - Cleaned: Solid line (ls="-", alpha=0.9, lw=1.6)
Both use the EXACT SAME color per channel from the master 8-color palette!
"""
...
def process_dataset(cleaned_filename):
    ...
    raw_csv_path = os.path.join(RAW_WITH_DIR, raw_filename)
    ...
    print(f"Processing with_occ dataset: {base_name}...")
...
if __name__ == "__main__":
    ...
    print(f"Found {len(clean_files)} with_occ cleaned datasets.")
